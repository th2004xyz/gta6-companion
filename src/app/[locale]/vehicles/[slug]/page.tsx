...
import { getEntry, getAllEntries, splitLocaleContent, type VehicleFrontmatter } from "@/lib/content";
...
export function generateStaticParams() {
  const entries = getAllEntries<VehicleFrontmatter>("vehicles");
  return [
...
export async function generateMetadata({
  ...
  const entry = getEntry<VehicleFrontmatter>("vehicles", slug);
  if (!entry) return {};
  ...
  return (
    <main className="flex-1">
      <article className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        <DetailBreadcrumb
          items={[
            { label: t("title"), href: `/${locale}/vehicles` },
            { label: title },
          ]}
        />
